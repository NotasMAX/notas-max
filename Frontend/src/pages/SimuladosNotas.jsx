...
import {
  getOne as getOneSimulado,
  getSimuladosByTurma,
  atualizarConteudos,
} from "../api/simuladoApi";
...
export default function SimuladosNotas() {
  ...
  useEffect(() => {
    ...
    setProximoAluno(
      Turma.alunos[Turma.alunos.findIndex((a) => a._id === alunoURL) + 1] ||
        null
    );
    setAnteriorAluno(
      Turma.alunos[Turma.alunos.findIndex((a) => a._id === alunoURL) - 1] ||
    ...
    if (!simuladoURL && !alunoURL) {
      getSimuladosByTurma(turmaURL).then((response) => {
        const simulados = response.data.simulados;
        if (simulados.length > 0) {
          const firstSimuladoId = simulados[0]._id;
          navigate(
            `/Turmas/${turmaURL}/Simulados/${firstSimuladoId}/Notas/${
              Turma.alunos[0]._id || ""
            }`
          );
        }
    ...
    if (simuladoURL && !alunoURL) {
      navigate(
        `/Turmas/${turmaURL}/Simulados/${simuladoURL}/Notas/${
          Turma.alunos[0]._id || ""
        }`
      );
    }
  ...
  const handleFormSubmit = async (formData) => {
    try {
      const resultado = await atualizarConteudos(Simulado._id, {
        conteudos: formData.conteudos,
      });
      if (proximoAluno) {
        navigate(
          `/Turmas/${turmaURL}/Simulados/${simuladoURL}/Notas/${proximoAluno._id}`,
          {
            replace: true,
            state: {
              message:
                resultado.data?.message || "Notas atualizadas com sucesso",
              type: "success",
            },
          }
        );
      } else {
        navigate(`/Turmas/Info/${turmaURL}`, {
          replace: true,
          state: {
            message: resultado.data?.message || "Notas atualizadas com sucesso",
            type: "success",
          },
        });
        return;
    ...
    <div>
      ...
      <div className="flex justify-between">
        <div className={Style.SimuladosHeaderContainer}>
          <h2 className={StyleTitle.titlePage}>
            Lançamento de Notas - {Turma?.serie}º EM
          </h2>
          <p className={StyleTitle.subtitlePage}>
            Lançamento de notas em lote, insira o numero de acerto do aluno e
            clique em Salvar para ir para o proximo aluno da turma
          </p>
        </div>
        <div>
          {Simulado && Turma && (
            <>
              <BimestreAtual bimestre={Simulado.bimestre} ano={Turma.ano} />
            </>
          )}
        </div>
      </div>
      ...
      <div className={Style.SubHeaderContainer}>
        <h3 className={Style.SimuladosSubHeader}>
          {Aluno ? `${Aluno.nome} (${Aluno.email})` : "Carregando aluno..."}
        </h3>
