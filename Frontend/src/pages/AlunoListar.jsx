...
import { getAlunos } from '../api/usuariosapi';
...
export default function AlunoListar() {
	...
	useEffect(() => {
		...
		const fetchAlunos = async () => {
			try {
				...
				if (mounted) setAlunos(res.data.alunos || []);
			} catch (err) {
	...
	return (
		<div className={Style.pageContainer}>
			<h2 className={Style.pageHeader}>Aluno</h2>

			<div className={Style.headerContainer}>
				<a href="/Alunos/CadastrarAluno" className={Style.linkCadastrar}>
			...
			{alunos.length === 0 ? (
				...
				<div className={Style.tableWrapper}>
					<table className={Style.table}>
						...
						<tbody>
							{alunos.map((aluno) => (
								<tr key={aluno._id || aluno.id} className={Style.tableRow}>
									<td className={Style.tableCell}>{aluno.nome}</td>
									<td className={Style.tableCell}>{aluno.email}</td>
									<td className={Style.tableCell}>{aluno.telefone_contato}</td>
									<td className={Style.tableCellAction}>
										<div className={Style.actionButtons}>
											{/* Botão Visualizar */}
											<button
												className={Style.viewButton}
												onClick={() => navigate(`/Alunos/Visualizar/${aluno._id || aluno.id}`)}
											...
											<button
												...
												onClick={() => navigate(`/Alunos/Editar/${aluno._id || aluno.id}`)}
