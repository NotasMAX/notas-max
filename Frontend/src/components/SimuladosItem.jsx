...
import Style from "../styles/SimuladosItem.module.css";
import { Tooltip } from 'primereact/tooltip';

export default function SimuladosItem({ simulado }) {
    const tooltipRef = useRef(null);
    const isEditable = (Date.now() - new Date(simulado.createdAt).getTime()) < (16 * 24 * 60 * 60 * 1000);

    return (
        <div className={Style.ContainerSimuladoRow}>
            <Tooltip target={tooltipRef} />
            <div className={Style.ContainerSimuladoCol} >
            ...
            <div
                ref={tooltipRef}
                className={Style.ContainerSimuladoColAcoes}
                data-pr-tooltip={isEditable ? `Editar simulado Nº ${simulado.numero}` : 'Não é possível editar após 15 dias da criação'}
                data-pr-position="left"
                data-pr-at="left-5 center"
                data-pr-my="right center-2">
                <a className={isEditable ? Style.LinkSimulado : Style.LinkSimuladoDisabled} href={isEditable ? `/Simulados/Editar/${simulado._id}` : '#'}>
                    <svg className={Style.ContainerSimuladoColIcon} viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
