import React from 'react';
import Style from "../styles/List.module.css";
import Button from "../styles/ButtonGroup.module.css";

export default function SimuladoAlunoItem({ alunoResultado, onView }) {

    const resultados = () => {
        return alunoResultado.resultados.reduce((total, c) => {
            return total + (c.resultado?.acertos || 0);
        }, 0);
    }

    const questoes = () => {
        return alunoResultado.resultados.reduce((total, c) => {
            return total + (c.quantidade_questoes || 0);
        }, 0);
    }

    return (

        <div className="flex w-full bg-white border border-gray-300 rounded-lg py-2 px-3 items-center">
            <div className="w-1/3">
                {alunoResultado.nome}
            </div>

            <div className="flex w-1/3">
                <p className='w-1/12'>{ resultados()} </p>
                <p className='w-1/12'>/</p>
                <p className='w-1/12'>{questoes()}</p>
            </div>
            <div className="w-1/3">
                <button className={Button.buttonSquare} onClick={onView}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M12 8.25C9.92894 8.25 8.25001 9.92893 8.25001 12C8.25001 14.0711 9.92894 15.75 12 15.75C14.0711 15.75 15.75 14.0711 15.75 12C15.75 9.92893 14.0711 8.25 12 8.25ZM9.75001 12C9.75001 10.7574 10.7574 9.75 12 9.75C13.2426 9.75 14.25 10.7574 14.25 12C14.25 13.2426 13.2426 14.25 12 14.25C10.7574 14.25 9.75001 13.2426 9.75001 12Z" fill="#2D264B" />
                        <path fillRule="evenodd" clipRule="evenodd" d="M12 3.25C9.85905 3.25 7.92325 4.30899 6.35173 5.59572C4.77164 6.88946 3.47962 8.47865 2.61142 9.68801L2.53981 9.78759C2.01715 10.5138 1.58691 11.1117 1.58691 12C1.58691 12.8883 2.01715 13.4862 2.53981 14.2124L2.61142 14.312C3.47962 15.5214 4.77164 17.1105 6.35173 18.4043C7.92325 19.691 9.85905 20.75 12 20.75C14.141 20.75 16.0768 19.691 17.6483 18.4043C19.2284 17.1105 20.5204 15.5214 21.3886 14.312L21.4602 14.2124C21.9829 13.4862 22.4131 12.8883 22.4131 12C22.4131 11.1117 21.9829 10.5138 21.4602 9.78759L21.3886 9.68801C20.5204 8.47865 19.2284 6.88946 17.6483 5.59572C16.0768 4.30899 14.141 3.25 12 3.25ZM3.82993 10.5628C4.6592 9.40765 5.86348 7.93414 7.302 6.75631C8.7491 5.57146 10.3542 4.75 12 4.75C13.6458 4.75 15.2509 5.57146 16.698 6.75631C18.1365 7.93414 19.3408 9.40765 20.1701 10.5628C20.794 11.4319 20.9131 11.6415 20.9131 12C20.9131 12.3585 20.794 12.5681 20.1701 13.4372C19.3408 14.5923 18.1365 16.0659 16.698 17.2437C15.2509 18.4285 13.6458 19.25 12 19.25C10.3542 19.25 8.7491 18.4285 7.302 17.2437C5.86349 16.0659 4.6592 14.5923 3.82993 13.4372C3.20597 12.5681 3.08691 12.3585 3.08691 12C3.08691 11.6415 3.20597 11.4319 3.82993 10.5628Z" fill="#2D264B" />
                    </svg>
                </button>
            </div>
        </div>
    )
};