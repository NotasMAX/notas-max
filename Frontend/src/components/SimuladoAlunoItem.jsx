...
export default function SimuladoAlunoItem({ alunoResultado, onView }) {
  ...
  return (
    <div className="flex w-full bg-white border border-gray-300 rounded-lg py-2 px-3 items-center">
      <Tooltip target={tooltipRef} position="top" mouseTrackTop={15} />

      <div className="w-1/3">{alunoResultado.nome}</div>
      ...
      <div className="w-1/3">
        <button
          ...
          onClick={onView}
          ref={tooltipRef}
          data-pr-tooltip={`Ver as notas do aluno ${alunoResultado.nome}`}
        >
          <svg
            width="24"
            height="24"
